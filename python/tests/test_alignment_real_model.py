...
@pytest.mark.skipif(os.environ.get("OPEN_KARAOKE_REAL_ALIGNMENT") != "1",
...
def test_actual_japanese_acoustic_model(tmp_path):
    ...
    atomic_json(tmp_path / "lyrics.json", {"format_version": 1, "analysis_version": 1,
                "segments": [{"id": "smoke", "text": "あ", "start_sample": 0, "end_sample": 48000}]})
    context = AnalysisContext(tmp_path, "jobs/real/alignment", tmp_path / "vocals.wav",
                              Event(), lambda *_: None)
    ...
    atomic_json(directory / "real-alignment-smoke.json", {
        "model": report["model"], "inference_performed": True,
        "fixture": "generated-220hz-tone-v1", "input_sha256": report["input_sha256"],
        "artifacts": [Path(path).name for path in paths],
        "singing_accuracy_verified": False, "quality_gate_applied": False,
        "note": "Forced target on a tone exercises integration, not recognition/alignment accuracy."})
