...
def inputs(root):
    ...
    lyrics = {
        "format_version": 1,
        "analysis_version": 1,
        "segments": [{"id": "s", "text": "あい", "start_sample": 0, "end_sample": 48000}],
    }
...
def test_api_results_and_isolated_output(tmp_path):
    ...
    with TestClient(
        create_app(tmp_path, TOKEN, [AlignmentAdapter(FixtureBackend)]), headers=HEADERS
    ) as session:
        ...
        assert all(
            path.startswith("jobs/") and (tmp_path / path).is_file() for path in job["artifacts"]
        )
...
def test_cancel_worker_cleanup(tmp_path):
    ...
    with TestClient(
        create_app(tmp_path, TOKEN, [AlignmentAdapter(Slow)]), headers=HEADERS
    ) as session:
...
def test_failure_is_structured_and_cleans_outputs(tmp_path):
    ...
    with TestClient(
        create_app(tmp_path, TOKEN, [AlignmentAdapter(Broken)]), headers=HEADERS
    ) as session:
        identifier = session.post("/analysis/alignment", json=body).json()["id"]
