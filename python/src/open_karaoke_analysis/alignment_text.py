...
def read_reference(context: AnalysisContext, relative: str | None) -> dict | None:
    ...
    try:
        ...
        raise ServiceError(
            "invalid_reference", "Imported lyrics must be UTF-8 plain text"
        ) from error
    ...
    for start, end in clusters:
        for char in unicodedata.normalize("NFKC", text[start:end]).casefold():
            if char.isspace() or unicodedata.category(char).startswith("P"):
                continue
            chars.append(
                {"char": char, "char_start": start, "char_end": end, "normalized_index": len(chars)}
            )
    return chars
...
def map_reference(segments: list[dict], reference: dict | None, checkpoint) -> dict | None:
    ...
    if reference is None:
        return None
    asr = [
        {**char, "segment_id": segment["id"]}
        for segment in segments
        for char in normalized_chars(segment["text"])
    ]
    ref = [
        {**char, "line_id": line["id"]}
        for line in reference["lines"]
        for char in normalized_chars(line["text"])
    ]
    ...
    for tag, a0, a1, b0, b1 in SequenceMatcher(None, a, b, autojunk=False).get_opcodes():
        ...
        if tag == "equal":
            matches.extend(
                {"asr_index": i, "reference_index": j}
                for i, j in zip(range(a0, a1), range(b0, b1), strict=True)
            )
    return {
        "normalization": "NFKC-casefold-ignore-punctuation-and-whitespace-v1",
        "asr_characters": asr,
        "reference_characters": ref,
        "operations": operations,
        "matches": matches,
        "reference_is_truth": False,
        "matching_is_correctness_probability": False,
    }
...
def reference_timeline(reference: dict, mapping: dict, lines: list[dict]) -> list[dict]:
    ...
    aligned = {(line["id"], c["normalized_index"]): c for line in lines for c in line["characters"]}
    match = {item["reference_index"]: item["asr_index"] for item in mapping["matches"]}
    results = []
    texts = [
        "".join(c["char"] for c in normalized_chars(line["text"])) for line in reference["lines"]
    ]
    ...
    for line, normalized in zip(reference["lines"], texts, strict=True):
        ...
        for index, char in grouped.get(line["id"], []):
            ...
            observed = (
                aligned.get((source["segment_id"], source["normalized_index"])) if source else None
            )
            ...
            units.append(
                {
                    **char,
                    "source": source,
                    "status": "matched" if timed else "unaligned",
        ...
        publish = complete and contiguous and ordered
        flags = []
        if complete and not ordered:
            flags.append("nonmonotonic_reference_timing_review")
        ...
        results.append(
            {
                **line,
                "status": "matched" if publish else "unaligned",
                "start_sample": units[0]["start_sample"] if publish else None,
                "end_sample": units[-1]["end_sample"] if publish else None,
                "characters": units,
                "flags": flags,
