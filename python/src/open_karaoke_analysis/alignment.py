...
def aggregate_units(text: str, characters: list[dict], kind: str, segment_id: str) -> list[dict]:
    ...
    for index, (start, end) in enumerate(ranges):
        ...
        units.append({"id": f"{segment_id}:unit-{index + 1:04d}", "text": text[start:end],
                      "char_start": start, "char_end": end, "unit_kind": kind,
...
def align_segment(segment: dict, emissions, vocabulary: dict, blank_id: int,
                  options: dict, context: AnalysisContext) -> dict:
    chars = [{**char, "start_sample": None, "end_sample": None,
              "status": "unaligned", "score": None}
             for char in normalized_chars(segment["text"])]
    line = {"id": segment["id"], "text": segment["text"], "characters": chars,
            "asr_start_sample": segment["start_sample"], "asr_end_sample": segment["end_sample"],
            "start_sample": None, "end_sample": None, "status": "unaligned",
            "timing_confidence": None, "flags": []}
    reason = None
    if not chars:
        ...
        line["unsupported_characters"] = sorted({c["char"] for c in chars if c["char"] not in vocabulary})
    else:
        spans = align_tokens(emissions, [vocabulary[c["char"]] for c in chars], blank_id,
                             context.checkpoint)
        if spans is None:
            reason = "no_ctc_path"
        else:
            for char, span in zip(chars, spans, strict=True):
                start, end = frame_samples(span.start_frame, span.end_frame, len(emissions),
            ...
            if all(char["status"] == "aligned" for char in chars):
                line.update(status="aligned", start_sample=chars[0]["start_sample"],
...
class AlignmentAdapter:
    info = ModelInfo(id="wav2vec2-ja-alignment", version=f"ctc-v1@{REVISION}", role=Role.ALIGNMENT,
                     capabilities=["ja", "ctc_alignment", "line_timing", "character_timing",
                                   "whitespace_word_timing", "reference_mapping"])
    ...
    def analyze(self, context: AnalysisContext, options: dict[str, Any]) -> list[str]:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix=".alignment-", dir=root) as temp:
                ...
                with context.input_path.open("rb") as source, path.open("wb") as target:
                    while block := source.read(1024 * 1024):
                        ...
                        if target.tell() + len(block) > MAX_DURATION * 8 + 1_048_576:
                            raise ServiceError("invalid_audio", "Alignment input exceeds 1 h size limit")
                        digest.update(block)
                        target.write(block)
                try:
                ...
                with audio:
                    if (audio.format not in {"WAV", "WAVEX", "RF64"} or audio.samplerate != RATE
                            or audio.subtype != "FLOAT" or audio.channels not in {1, 2}
                            or not 0 < audio.frames <= MAX_DURATION):
                        raise ServiceError("invalid_audio", "Expected 48 kHz float32 mono/stereo WAV")
                    duration = audio.frames
                    if any(s["end_sample"] > duration or s["end_sample"] - s["start_sample"] > 30 * RATE
                           for s in original["segments"]):
                        raise ServiceError("invalid_lyrics", "ASR windows must fit audio and be <=30 s")
                    ...
                    for index, segment in enumerate(original["segments"]):
                        ...
                        if non_silent and len(mono) >= 1200 and normalized_chars(segment["text"]):
                            if not prepared:
                                ...
                                prepared = True
                            context.checkpoint()
                            emissions = backend.emissions(resample_poly(mono, 1, 3).astype(np.float32))
                            context.checkpoint()
                            inference_count += 1
                        line = align_segment(segment, emissions, backend.vocabulary, backend.blank_id,
                                             options, context)
                        ...
                        lines.append(line)
                        context.report(0.1 + 0.8 * (index + 1) / len(original["segments"]), "aligning_lyrics")
                canonical, items = [], []
                for segment, line in zip(original["segments"], lines, strict=True):
                    timed = line["status"] == "aligned"
                    canonical.append({"id": segment["id"], "text": segment["text"],
                                      "start_sample": line["start_sample"] if timed else segment["start_sample"],
                    ...
                    items.append({"target_id": segment["id"], "evidence": {
                        "timing_kind": "ctc_forced" if timed else "coarse_asr_fallback",
                        "original_source": segment.get("source"), "flags": line["flags"],
                        "confidence_calibrated": False, "alignment": line}})
                units = [unit for line in lines for unit in line["words"]]
                metadata = {"model": backend.identity(), "pipeline_version": 1,
                            "sample_rate": RATE, "duration_samples": duration,
                            "input_sha256": digest.hexdigest(), "lyrics_sha256": lyrics_hash,
                            "options": options, "inference_performed": inference_count > 0,
                            "aligned_units": sum(u["status"] == "aligned" for u in units),
                            "total_units": len(units), "candidate_only": True,
                            "singing_accuracy_verified": False, "confidence_calibrated": False}
                version = {"format_version": 1, "analysis_version": options["analysis_version"]}
                details = {**version, **metadata, "asr_original": original, "lines": lines,
                           "reference": reference, "reference_mapping": mapping,
                           "reference_lines": reference_timeline(reference, mapping, lines)
                           if reference is not None else []}
                evidence = {**version, "items": [{"target_id": "lyrics", "evidence": metadata}, *items]}
                return [context.write_json(name, value) for name, value in (
                    ("lyrics.json", {**version, "segments": canonical}),
                    ("alignment.json", details), ("alignment_evidence.json", evidence))]
        except OSError as error:
            raise ServiceError("artifact_io", "Could not read or write alignment artifacts", 500) from error
        finally:
            backend.close()
