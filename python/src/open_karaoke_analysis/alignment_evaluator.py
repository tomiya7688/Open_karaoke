...
def evaluate(dataset: dict, *, max_mean_ms=None, max_p95_ms=None, min_coverage=None) -> dict:
    ...
    if min_coverage is not None and (isinstance(min_coverage, bool) or not math.isfinite(min_coverage)
                                     or not 0 <= min_coverage <= 1):
        raise ValueError("Coverage must be between zero and one")
    if (not isinstance(dataset, dict) or not isinstance(dataset.get("dataset_version"), str)
            or not dataset["dataset_version"] or not isinstance(dataset.get("items"), list)
    ...
    passed = not applied or (bool(errors) and coverage >= required_coverage
                            and (max_mean_ms is None or mean <= max_mean_ms)
                            and (max_p95_ms is None or p95 <= max_p95_ms))
    return {"evaluator": "lyric-alignment", "version": 1, "sample_rate": 48_000,
            "dataset_version": dataset["dataset_version"], "reference_units": len(ids),
            "matched_units": len(ids) - len(missing), "missing_ids": missing,
            "coverage": coverage, "mean_boundary_error_ms": mean,
            "p95_boundary_error_ms": p95, "p95_method": "linear",
            "quality_gate_applied": applied, "passed": passed,
            "thresholds": {"max_mean_ms": max_mean_ms, "max_p95_ms": max_p95_ms,
...
def main() -> int:
    ...
    try:
        ...
        result = evaluate(json.loads(raw), max_mean_ms=args.max_mean_ms,
                          max_p95_ms=args.max_p95_ms, min_coverage=args.min_coverage)
