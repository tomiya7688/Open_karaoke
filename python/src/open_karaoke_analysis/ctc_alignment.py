...
def align_tokens(log_probs, tokens: list[int], blank_id: int, checkpoint) -> list[TokenSpan] | None:
    ...
    if (
        not np.isfinite(values).all() or np.any(values > 1e-5)
        or not np.allclose(np.exp(values).sum(axis=1), 1, atol=1e-4)
        or type(blank_id) is not int or not 0 <= blank_id < classes
        or any(type(t) is not int or t == blank_id or not 0 <= t < classes for t in tokens)
    ):
        raise ServiceError("invalid_emissions", "Expected normalized CTC log probabilities and labels")
    if not tokens:
        return []
    states = 2 * len(tokens) + 1
    if frames * states > MAX_CELLS:
        raise ServiceError("alignment_too_large", "CTC alignment exceeds the bounded trellis budget")
    ...
    if any(not item for item in positions):
        return None
    return [TokenSpan(min(pos), max(pos) + 1, float(np.exp(values[pos, token]).mean()))
            for pos, token in zip(positions, tokens, strict=True)]


def frame_samples(start_frame: int, end_frame: int, frame_count: int,
