...
FILES = {
    "config.json": 131_072,
    "preprocessor_config.json": 16_384,
    "vocab.json": 131_072,
    ...
    try:
        with os.fdopen(fd, "wb") as output:
            ...
            try:
                ...
                raise ServiceError(
                    "model_download", "Alignment model download failed", 503, True
                ) from error
            output.flush()
    ...
    if manifest_path.is_file():
        try:
            ...
            for name, limit in FILES.items():
                expected = (
                    WEIGHT_SHA256 if name == "pytorch_model.bin" else manifest["sha256"][name]
                )
                if digest_file(paths[name], context, limit) != expected:
                    raise ValueError("invalid hash")
        except (OSError, ValueError, KeyError, TypeError) as error:
            raise ServiceError(
                "model_integrity", "Alignment-model snapshot is incomplete or corrupt"
            ) from error
        return directory, manifest
    if not allow_download:
        raise ServiceError(
            "model_not_installed", "A verified alignment model is required; downloads disabled", 503
        )
...
class JapaneseCtcBackend:
    ...
    def prepare(self, context, options: dict) -> None:
        ...
        try:
            ...
            if (
                self.processor.sampling_rate != 16_000
            ...
            self.vocabulary = {
                char.casefold(): index for char, index in vocab.items() if len(char) == 1
            }
            self.model = model.eval().to(options["device"])
            self.device = options["device"]
        except (ValueError, RuntimeError, OSError, KeyError) as error:
            raise ServiceError(
                "model_load_failed", "Could not load the verified alignment model", 503
            ) from error
        context.checkpoint()
